...
if ($contentType === "application/json") {
  ...
  if( is_array($decoded)) {
	...
	$fpr=$decoded['fingerprint'];
	$val->reactions = $array;
	$val->active = GetActive($fpr);
	$val->error=null;
...
function GetActive($fpr){
	...
	try {
		...
		// set the PDO error mode to exception
		$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$stmt = $conn->prepare("SELECT emoji FROM users WHERE fingerprint='".$fpr."'"); 
		$stmt->execute();
		$result_a = $stmt->fetch(PDO::FETCH_ASSOC);
		$result=$result_a['emoji'];
    }
	catch(PDOException $e){
		$result ="Connection failed: " . $e->getMessage();
    }
